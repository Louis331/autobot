  ...
  client.on('message', msg => {
        if (msg.channel.id != '598806292156514335'){
            if (msg.content.toLowerCase().startsWith(`${prefix}startmovie`) && !msg.author.bot && msg.member.hasPermission('ADMINISTRATOR')){
                ...
                if (command.length > 2){
                    name = command.slice(2).join(' ')
                    msg.channel.send(`@everyone Movie event incoming. Will be watching ${name}. Starting at ${command.slice(1,2)}. Reply to this message to get a movie ticket`)
                }else{
